...
# 🔄 Auto-refresh ogni 30 secondi (ridotto per evitare troppe richieste)
...
# Configura cache per ridurre le richieste all'API
cache = TTLCache(maxsize=100, ttl=30)  # Cache per 30 secondi
...
def get_sheet_data():
    ...
    try:
        ...
        st.error(f"Errore API Google Sheets: {e}")
        return pd.DataFrame(), 0
    except Exception as e:
        st.error(f"Errore imprevisto: {e}")
        return pd.DataFrame(), 0

# 📥 Dati con gestione errori
try:
    ...
    **Le nuove risposte si evidenziano con un effetto pulsante per i primi 10 secondi.**
    ...
    st.error("Si è verificato un errore nel caricamento dell'applicazione.")
    st.info("Ricarica la pagina per riprovare.")
    # Mostra una versione statica di fallback se disponibile
    st.markdown("""
    <div style='text-align: center; color: white; padding: 2rem;'>
        <h2>Specchio Empatico</h2>
        <p>L'applicazione sta incontrando problemi temporanei di connessione.</p>
        <p>Riprova tra qualche momento.</p>
    </div>
    """, unsafe_allow_html=True)
